...
class jsonOptionsAdmin {
  ...
  function  __construct() {
    ...
    $this->fields = array(
      // Main Fields
      array(
        'name' => jsonOptions::$name . '_filter_fields',
      ...
      array(
		'name' => jsonOptions::$name . '_filter_remove',
		'title' => 'Remove Filter',
		'description' => 'Choose a filter to remove.',
		'type' => 'select',
		'default' => array('janrain_capture','rpx'),
      	'selected' => 'disabled',
		'options' => get_option(jsonOptions::$name . '_filters_available'),
		'screen' => 'main',
	   ),
       array(
		'name' => jsonOptions::$name . '_filter_add',
		'title' => 'Add Filter',
     ...
     // import screen
      ...
      array(
        'name' => jsonOptions::$name . '_filters_enabled',
      ...
      array(
		'name' => jsonOptions::$name . '_filters',
      ...
      array(
        'name' => jsonOptions::$name . '_filter_combine',
        'title' => 'Combine Type',
     ...
     array(
        'name' => jsonOptions::$name . '_filter_compare',
      ...
      array(
        'name' => jsonOptions::$name . '_import_fields',
        'title' => 'Import Settings',
        'type' => 'title',
        'screen' => 'import',
      ),
      array(
        'name' => jsonOptions::$name . '_upload',
        'title' => 'Import File',
     ...
     array(
       ...
       array(
        'name' => jsonOptions::$name . '_export_fields',
        'title' => 'Export Settings',
        'type' => 'title',
        'screen' => 'export',
      ),
       array(
        'name' => jsonOptions::$name . '_action',
        'title' => 'Action',
  ...
  function admin_menu() {
    $optPage = add_menu_page(
    	...
    	'manage_options', jsonOptions::$name . '', array( &$this, 'main' )
    );
    $importPage = add_submenu_page(
          jsonOptions::$name . '', __( 'JSON Options' ), __( 'Import' ),
          'manage_options', jsonOptions::$name . '_import', array( &$this, 'import' )
    );
    $exportPage = add_submenu_page(
          jsonOptions::$name . '', __( 'JSON Options' ), __( 'Export' ),
          'manage_options', jsonOptions::$name . '_export', array( &$this, 'export' )
    );
  }
...
HIDDEN;
  ...
  public function on_post() {
  	global $wpdb;
  	$this->filters = isset($_POST[jsonOptions::$name . '_filters']) ? $_POST[jsonOptions::$name . '_filters'] : '';
  	$this->filter = isset($_POST[jsonOptions::$name . '_filters_enabled']) ? true : false;
  	
  	// importing
  	if( $_FILES && $_FILES[jsonOptions::$name .'_upload'] ) {
  		$this->import_JSON();
  	}
  	// exporting
  	if ( isset($_POST[jsonOptions::$name . '_action']) ) {
  		$this->action = $_POST[jsonOptions::$name . '_action'];
  		if( $this->action ) {
  			$this->export_JSON();
  		}
  	} else { // managing filters
  		$this->filters = get_option(jsonOptions::$name . '_filters_available');
  		if ( ! is_array( $this->filters ) ) {
  			$this->filters = array('janrain_capture','rpx');
  		}
  		if (isset($_POST[jsonOptions::$name . '_filter_add']) && $_POST[jsonOptions::$name . '_filter_add'] != '') {
  			//validate
  			$filter = $_POST[jsonOptions::$name . '_filter_add'];
  			$valid = array('-', '_');
  			if(!ctype_alnum(str_replace($valid, '0',$filter))){
  				...
  				update_option(jsonOptions::$name . '_filters_available', $this->filters);
  				$this->postMessage = array( 'class' => 'updated', 'message' => "Filter Added: $filter" );
  			}
  		}
  	  	if ( isset( $_POST[jsonOptions::$name . '_filter_remove'] )
  	  	         && $_POST[jsonOptions::$name . '_filter_remove'] != '' ) {
  			//remove filter
  			$filter = $_POST[jsonOptions::$name . '_filter_remove'];
  ...
  private function save_fields() {
    ...
    private function import_JSON() {
    	global $wpdb;
    	$where = '';
    	if ( $_FILES[jsonOptions::$name . "_upload"]["error"] > 0 ) {
    		$this->postMessage = array( 'class' => 'error', 'message' => 'Import Failed - ' . $_FILES["file"]["error"] );
	 	} else {
			$this->json = file_get_contents($_FILES[jsonOptions::$name . "_upload"]['tmp_name']);
	  	}
    	...
    	if ( is_string( $this->json ) ) {
    		...
    		foreach ( $this->json as $k => $v ) {
	      		$k = mysql_real_escape_string($k);
	   	  		...
	   	  		$combine = $_POST[jsonOptions::$name . '_filter_combine'];
	   		 	$compare = $_POST[jsonOptions::$name . '_filter_compare'];
  	...
  	private function export_JSON() {
		...
		switch ($this->action) {
	    	case 'Download':
	    		header( 'Content-Disposition: attachment; filename="' . jsonOptions::$name . '_' . date('Y-m-d_h.i.s', time()) . '_WP-' . get_bloginfo('version') . '.json"');
